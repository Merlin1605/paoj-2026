...
public class LibraryService {
    ...
    // ==========================================
    // TRANZACȚII EXPLICITE (Commit / Rollback)
    // ==========================================
    ...
    public long borrowBook(long readerId, long bookId) throws SQLException, IOException {
        ...
        try {
            ...
            conn.commit(); // Salvăm totul definitiv
            System.out.println("[TX] borrowBook comis cu succes. Loan ID=" + loanId);
            return loanId;

        } catch (SQLException e) {
            conn.rollback(); // Ceva a picat, dăm înapoi toate modificările!
            System.out.println("[TX] borrowBook — rollback datorita: " + e.getMessage());
            throw e;
        } finally {
            conn.setAutoCommit(true); // Revenim la normal
        }
    }
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(findSql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next()) throw new SQLException("Imprumutul nu exista.");
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql);
            ...
            while (rs.next()) {
                results.add(String.format("%s: %d imprumuturi",
                        rs.getString("reader_name"), rs.getLong("total_loans")));
            }
        }
        return results;
    }
